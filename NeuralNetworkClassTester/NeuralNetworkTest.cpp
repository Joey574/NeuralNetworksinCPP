...
void NeuralNetwork::Compile(loss_metrics loss, loss_metrics metrics, optimization_technique optimizer, Matrix::init weight_initialization) {
	...
	for (int i = 0; i < network_dimensions.size() - 1; i++) {
		if (res_net_layers.find(i) != res_net_layers.end()) {
			current_network.weights.emplace_back(network_dimensions[i] + network_dimensions[0], network_dimensions[i + 1], 0.5f);
		}
		else {
			current_network.weights.emplace_back(network_dimensions[i], network_dimensions[i + 1], 0.5f);
		}
...
NeuralNetwork::training_history NeuralNetwork::Fit(Matrix x_train, Matrix y_train, int batch_size, int epochs, float learning_rate, float validation_split, bool shuffle, int validation_freq) {
	...
	for (int i = 0; i < current_network.weights.size(); i++) {
		std::cout << "weights[" << i << "]" << current_network.weights[i].Size() << current_network.weights[i].ToString();
	}
	std::cout << std::endl;

	for (int i = 0; i < current_network.weights.size(); i++) {
		std::cout << "biases[" << i << "] " << current_network.biases[i].size() << std::endl;
		for (int x = 0; x < current_network.biases[i].size(); x++) {
			std::cout << current_network.biases[i][x] << " ";
		} std::cout << std::endl;
	}
	std::cout << std::endl;
	...
	for (int e = 0; e < epochs; e++) {
		...
		for (int i = 0; i < iterations; i++) {
			...
			std::cout << "X: " << x.Size() << x.ToString();
			std::cout << "Y: " << y.Size() << y.ToString() << std::endl;

			current_results = forward_propogate(x, current_network, current_results);

			for (int x = 0; x < current_results.total.size(); x++) {
				std::cout << "Total[" << x << "] " << current_results.total[x].Size() << current_results.total[x].ToString();
				std::cout << "Activation[" << x << "] " << current_results.activation[x].Size() << current_results.activation[x].ToString() << std::endl;
			}
...
NeuralNetwork::network_structure  NeuralNetwork::backward_propogate(Matrix x, Matrix y, float learning_rate, network_structure net, result_matrices results, derivative_matrices deriv) {
	...
	std::cout << "\n\nTotal Error: " << deriv.d_total[deriv.d_total.size() - 1].Size() << deriv.d_total[deriv.d_total.size() - 1].ToString();
